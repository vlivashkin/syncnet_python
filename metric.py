...
class SyncNetMetric:
    def __init__(self, video_path: str, name: str = None, data_dir='./temp', device='cuda:0'):
        if name is None:
            name = video_path.rsplit('/', 1)[1].rsplit('.', 1)[0]
        self.opt = Config(videofile=video_path, reference=name, data_dir=data_dir)
        self.device = device
    ...
    def _inference(self):
        ...
        s.load_parameters(self.opt.initial_model)
        print("Model %s loaded." % self.opt.initial_model)
...
if __name__ == "__main__":
    ...
    metric = SyncNetMetric(video_path=args.video_path, name=args.name, data_dir=args.data_dir, device=args.device)
